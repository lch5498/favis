...
## 8. Sign in with Apple 설정

체키는 카카오 로그인과 함께 `Sign in with Apple`을 제공합니다. App Store 공개 심사에서는 Apple App Review Guideline 4.8에 따라, 제3자/소셜 로그인으로 계정을 만드는 앱이 동등한 대체 로그인 옵션을 제공해야 합니다.

현재 구현 상태:

- Flutter 앱 로그인 화면에 `Apple로 계속하기` 버튼 표시
- Apple identity token을 Next.js API `/api/mobile/auth/apple`로 전달
- API 서버에서 Apple 공개키로 identity token 검증
- `authentications.oauth_provider = 'apple'`로 사용자 계정 연결
- 기존 자체 session token 반환 구조 재사용
- iOS `Runner.entitlements`에 `com.apple.developer.applesignin` 설정
...
Xcode/App Store Connect에서 확인할 것:

- Apple Developer > Identifiers > `com.family.checky.mobile`에서 `Sign in with Apple` capability 활성화
- Xcode > Runner target > Signing & Capabilities에 `Sign in with Apple` 표시 확인
- Team과 Bundle ID가 `com.family.checky.mobile`인지 확인
- Vercel 환경 변수 `APPLE_CLIENT_ID`는 기본값과 다르게 쓸 때만 설정
...
Apple 로그인은 iOS/iPadOS에서 사용 가능할 때만 앱에 노출됩니다. Android에서는 카카오 로그인만 표시됩니다.
...
- [ ] Apple Developer Identifier에서 Sign in with Apple capability 활성화
- [ ] release/TestFlight에서 Apple 로그인 확인
...
### Guideline 4.8 - Login Services 리젝

카카오 같은 제3자 로그인을 제공하면서 Apple 로그인 또는 동등한 대체 로그인이 없으면 Guideline 4.8로 리젝될 수 있습니다.

체키 대응:

- 로그인 화면에 `Apple로 계속하기` 버튼 제공
- Apple 로그인도 최초 가입 시 닉네임 설정 후 동일한 기능 접근 가능
- App Review Notes에 `Apple 로그인 또는 카카오 로그인으로 테스트할 수 있습니다.`라고 작성
